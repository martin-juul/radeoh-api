...
<table class="table table-sm table-hover" style="visibility: hidden;">
    ...
    @foreach ($routes as $route)
        <tr>
            <td>
                @foreach (array_diff($route->methods(), config('dev-route-explorer.hide_methods')) as $method)
            ...
            <td><a href="/{{ $route->uri() }}" target="_blank">Go</a></td>
        </tr>
    @endforeach
    </tbody>
</table>
...
<script type="text/javascript">
    function hideEmptyDomainColumn() {
        ...
        const domains = table.querySelectorAll('tbody .domain');
        const emptyDomains = table.querySelectorAll('tbody .domain-empty');
